                    ...
                    >
                      <div className="flex">
                        <img
                          src={imageToUse}
                          alt={restaurant.name}
                          className="w-20 h-20 xs:w-24 xs:h-24 object-cover flex-shrink-0"
                          onError={(e) => {
            ...
            <div className="grid grid-cols-2 gap-3 pb-4">
              {filteredGroceryItems.length > 0 ? (
                    ...
                    >
                      {hasImage ? (
                        <img
                          src={item.imageUrl}
                          alt={item.name}
                          className="w-full h-28 xs:h-32 object-cover mb-2 rounded-lg"
                          onError={(e) => (e.target.style.display = 'none')}
                        />
                      ) : (
                        <div className="flex items-center justify-center w-full h-28 xs:h-32 mb-2 bg-gradient-to-br from-green-50 to-green-100 text-3xl xs:text-4xl rounded-lg">
                          {item.unicode || '🛒'}
                        </div>
                      )}
                ...
                <div className="col-span-2 text-center py-12 px-4">
                  ...
                  {searchQuery && (
                    <p className="text-gray-400 text-xs mb-4 work-sans-medium text-pretty">
                      Try adjusting your search terms or browse all grocery items
                    </p>
                  )}
                  {searchQuery && (
                    <button
